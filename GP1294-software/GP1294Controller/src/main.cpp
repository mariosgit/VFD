...
void loop()
{
    if(redraw > 1000/20)
    {
        ...
        textPos += textAdd;
        if (textPos > 233) textAdd = -1;
        if (textPos < 195) textAdd = 1;
        ...
        for(int i = 0; i < 4; i++)
        {
            if(levelR[i] > levelRmax) levelRmax = levelR[i];
            levelRavr += levelR[i];
        }
        for(int i = 0; i < 4; i++)
        {
            if(levelL[i] > levelLmax) levelLmax = levelL[i];
            levelLavr += levelL[i];
        }
